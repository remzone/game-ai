...
import { monsterStats, monsterNames } from './ecology.js';
...
export function startBattle(w: World, enemyArmy?: string, siege?: string) {
  ...
  const defenders = enemyArmy ? w.armies.find((a) => a.id === enemyArmy) : undefined;
  if (defenders)
    for (const [i, id] of defenders.members.entries())
      fighters.push({
        id: `person:${id}`,
        person: id,
        side: 'enemy',
        ...
        maxHp: 100,
        x: 14 + (i % 6) * 0.7,
        y: 2 + Math.floor(i / 6) * 1.3,
        targetX: 8,
        targetY: 5,
        cooldown: 0,
        order: 'attack',
      });
  for (let i = 0; i < (defenders ? 0 : s.monsters); i++)
    fighters.push({
      id: `${s.monsterKind}:${i}`,
      side: 'enemy',
      unitClass: monsterStats[s.monsterKind].unitClass,
      hp: monsterStats[s.monsterKind].hp,
      maxHp: monsterStats[s.monsterKind].hp,
      x: 14 + (i % 3),
      y: 3 + Math.floor(i / 3),
      targetX: 8,
      targetY: 5,
...
export function finishBattle(w: World, result: 'victory' | 'defeat' | 'retreated') {
  ...
  b.status = result;
  w.speed = 0;
  const killed = b.fighters.filter(
    (f) => f.side === 'enemy' && f.hp <= 0 && f.person === undefined,
  ).length;
  ...
  for (const f of b.fighters)
    if (f.person !== undefined) {
      ...
      if (f.hp <= 0) death(w, person, b.enemyArmy ? 'штурм' : 'бой с существами');
      else person.experience++;
    }
  const army = w.armies.find((a) => a.id === p.army)!;
  army.members = army.members.filter((id) => w.people[id].alive);
  army.mounts = Math.min(
    army.mounts ?? 0,
    army.members.filter((id) => (w.people[id].unitClass ?? army.unitClass) === 'cavalry').length,
  );
  if (result === 'victory') {
    for (const q of w.quests)
      if (
        !b.enemyArmy &&
        q.settlement === b.settlement &&
        q.type === 'hunt' &&
        q.status === 'accepted'
      )
        q.objectiveMet = true;
  ...
  event(
    w,
    'battle_end',
    `Бой окончен: ${result}. Убито существ: ${killed}; погибло людей: ${b.fighters.filter((f) => f.person !== undefined && f.hp <= 0).length}.`,
...
export function stepBattle(w: World, dt = 0.25) {
  ...
  for (const f of b.fighters) {
    ...
    if (!target) break;
    const exhausted =
      f.unitClass === 'mages' && f.person !== undefined && w.people[f.person].mana < 2;
    const cfg = exhausted ? { range: 1.4, damage: 6, speed: 1.1 } : stats[f.unitClass];
    f.cooldown = Math.max(0, f.cooldown - dt);
    const distance = Math.hypot(target.x - f.x, target.y - f.y);
    if (distance <= cfg.range && f.cooldown === 0) {
      if (f.unitClass === 'mages' && !exhausted && f.person !== undefined)
        w.people[f.person].mana -= 2;
      const bonus =
